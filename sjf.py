...
if check1 is 1 and check2 is 1:
   ...
   for j in range(0,no_of_process-1):
      btime=btime+process_queue[j][2]
      min=process_queue[start][2]
      index=start
      for l in range(index,no_of_process):
        if btime>=process_queue[l][1] and process_queue[l][2]<min:
...
for counter in xrange(no_of_process):
    ...
    print process_queue[counter][0],'\t\t',process_queue[counter][1],'\t\t',process_queue[counter][2],'\t\t',process_queue[counter][3],'\t\t',process_queue[counter][4]
