...
for count in xrange(total_burst_time+1):
   if count is 0:
      ...
      loop=0
      for index in xrange(no_of_process):
         if process_queue[index][2] >=1:
            loop=index
            break
      for index in xrange(no_of_process):
         if process_queue[index][2]>=1:
            if process_queue[index][2]<process_queue[loop][2]:
               loop=index
   small=loop
   if process_queue[small][2] is 0:
      check1=check1+1
   if process_queue[small][2]>0: 
      if source_check is 0:
         remaining_time_queue.append(process_queue[small][1])
         time_count=time_count+process_queue[small][1]
         source_check=source_check+1
      if source_check>0:
         if waiting_time[small] is -1:
            waiting_time[small]=time_count

         remaining_time_queue.append(process_queue[small][0])
         time_count=time_count+1
         remaining_time_queue.append(time_count)
         process_queue[small][2]=process_queue[small][2]-1
         if process_queue[small][2] is 0:

            remaining_time_queue.append('f')
            turn_time[small]=time_count
         else:
